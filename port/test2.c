...
void play_tone(int frequency, int duration_ms) { Beep(frequency, duration_ms); }
#elif __APPLE__
...
static OSStatus RenderTone(void * inRefCon, AudioUnitRenderActionFlags * ioActionFlags,
    ...
    double theta_increment =
        2.0 * M_PI * toneContext->frequency / toneContext->sampleRate;
...
void play_tone(int frequency, int duration_ms) {
    ...
    ToneContext toneContext = {
        .frequency = frequency, .sampleRate = 44100.0, .theta = 0};
    ...
    // Set the format
    AudioStreamBasicDescription streamFormat = {.mSampleRate = 44100,
    ...
    AudioUnitSetProperty(toneUnit, kAudioUnitProperty_StreamFormat,
